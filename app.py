...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgres://Bradley_Scharf@localhost/scharfbHW5"
...
def one_list(ident):
    ...
    #ADDITIONAL CHANGE:  SORTED IN ASCENDING ORDER BY PRIORITY NUMBER
    items = lst.items.order_by(asc(TodoItem.priority)).all()
...
def update(item):
    ...
    if form.validate_on_submit():
        updateItem = TodoItem.query.filter_by(id=item).first()
        updateItem.priority = form.priority.data
        db.session.commit()
        flash("Updated priority of " + updateItem.description)
        return redirect(url_for('all_lists'))
    return render_template('update_item.html', form=form, itemID=item)
...
def delete(lst):
    l = TodoList.query.filter_by(id=lst)
    name = l.first().title
